...
#pragma omp target map(tofrom:cultureLength,immunityLength,diseaseLength)
    //running this in parallel causes segementation fault(core dump)
...
Agent* Agent::reincarnate(std::pair<int,int> pos){
    ...
    //map to device(GPU)
// and run in parallel
// i,immunityLength - read/write
#pragma omp target device(0) map(tofrom:i,immunityLength)
...
//map to device(GPU)
// and run in parallel
// i,cultureLength - read/write
#pragma omp target device(0) map(tofrom:i,cultureLength)
#pragma omp parallel for
    for (int i=0; i<cultureLength; ++i) {

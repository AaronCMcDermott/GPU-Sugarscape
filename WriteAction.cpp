...
#pragma omp target device(0) map(to:size,i) map(tofrom:startX,startY)
...
#pragma omp critical(pushBack)
    ...
    // and run in parallel
    // FailedGroups.size() - read/write, i - read
#pragma omp target device(0) map(to:i,ExclusiveGroups.size()) map(from:ExclusiveGroups.size())
#pragma omp parallel for
...
#pragma omp parallel for
    ...
    // and run in parallel
    // FailedGroups.size() - read/write, i - read
#pragma omp target device(0) map(to:i,FailedGroups.size()) map(from:FailedGroups.size())
#pragma omp parallel for
...
bool WriteAction::concurrentRun(void){
    ...
    //offload to target device(GPU),
    // and run in parallel
    // totalTiles,tileNum,sectionSize - read/write, tileDim - read-only
#pragma omp target device(0) map(to:totalTiles,tileDim) map(from:totalTiles) map(tofrom:tileNum,sectionSize)
#pragma omp parallel for
...
int WriteAction::participantCount(int startX, int startY, int dimSize)
{
    ...
    //offload to target device(GPU),
    // and run in parallel
    // startX,startY - read/write, dimSize - read-only
#pragma omp target device(0) map(to:dimSize) (tofrom:startX,startY)
#pragma omp parallel for

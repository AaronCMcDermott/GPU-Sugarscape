...
/**
 * Constructor - pass World Pointer back to base class constructor
...
int IterativeWriteAction::participantCount(int startX, int startY, int dimSize)
{
    ...
    // and run in parallel
    // dimSize - read,  startX,startY - read/write
#pragma omp target device(0) map(to:dimSize) map(tofrom:startX,startY)

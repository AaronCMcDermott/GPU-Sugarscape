...
// and run in parallel
//startIndex,subject->getImmunityLength(),infection.size() all have read/write
#pragma omp target device(0) map(tofrom:startIndex,subject->getImmunityLength(),infection.size())
#pragma omp parallel for
                for (int startIndex = 0;
                     startIndex < subject->getImmunityLength() - infection.size(); ++startIndex) {
                    //!
                    /*!
...
// and run in parallel
//startIndex,subject->getImmunityLength(),infection.size() all have read/write
#pragma omp target device(gpu) map(tofrom:k,currentScore,,infection.size())
#pragma omp parallel for
